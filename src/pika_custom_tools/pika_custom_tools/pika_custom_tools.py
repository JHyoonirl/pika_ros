                    ...
                    try:
                        ...
                        if "AS5047" in root:
                            ...
                            if "error" not in as5047:
                                ...
                                angle = safe_float(as5047.get("rad"), 0.0)
                        ...
                        if "IMU" in root:
                            ...
                            roll = safe_float(imu_val.get("roll"))
                            pitch = safe_float(imu_val.get("pitch"))
                            yaw = safe_float(imu_val.get("yaw"))
                            imu_msg.orientation = euler_to_quaternion(roll, pitch, yaw)
                            
                            gyr = imu_val.get("gyr", [0,0,0])
                            imu_msg.angular_velocity.x = safe_float(gyr[0])
                            imu_msg.angular_velocity.y = safe_float(gyr[1])
                            imu_msg.angular_velocity.z = safe_float(gyr[2])
                            
                            acc = imu_val.get("acc", [0,0,0])
                            imu_msg.linear_acceleration.x = safe_float(acc[0])
                            imu_msg.linear_acceleration.y = safe_float(acc[1])
                            imu_msg.linear_acceleration.z = safe_float(acc[2])
                            
                            self.pub_imu.publish(imu_msg)

                        # ---------------------------------------------------------
                        # 3. Process Motor Status (DriverTemp, Voltage, etc.)
                        # ---------------------------------------------------------
                        if "motorstatus" in root:
                            ms_val = root["motorstatus"]
                            with self.receive_data_mtx:
                                # [수정] 모든 상태값에 safe_float 적용
                                self.voltage = safe_float(ms_val.get("Voltage"))
                                self.driver_temp = safe_float(ms_val.get("DriverTemp"))
                                self.motor_temp = safe_float(ms_val.get("MotorTemp"))
                                self.bus_current = safe_float(ms_val.get("BusCurrent"))
                                self.status_str = str(ms_val.get("Status", ""))
                            
                            status_hex_str = self.status_str
                            if len(status_hex_str) >= 4:
                                try:
                        ...
                        # ---------------------------------------------------------
                        # 4. Process Motor Control Data (Position, Speed, Current)
                        # ---------------------------------------------------------
                        if "motor" in root:
                            ...
                            # [수정] Position 안전 변환
                            angle = safe_float(motor_val.get("Position"))
                            gripper_msg.error = False
                            ...
                            # [수정] Current(Effort), Speed(Velocity) 안전 변환
                            gripper_msg.effort = safe_float(motor_val.get("Current"))
                            gripper_msg.velocity = safe_float(motor_val.get("Speed"))
                            
                            gripper_msg.enable = self.enable
                            # 저장된 상태값 사용 (위에서 safe_float로 처리됨)
                            ...
                            # [수정] JointState 필드 채우기 (float 리스트)
                            js_msg.position = [float(gripper_msg.distance)]
                            js_msg.velocity = [float(gripper_msg.velocity)]
                            js_msg.effort = [float(gripper_msg.effort)]
                            
                            # 로그는 필요할 때만 주석 해제 (WARN 방지)
                        ...
                        # ---------------------------------------------------------
                        if "Command" in root:
                            cmd_val = int(safe_float(root.get("Command"))) # Command도 안전하게
                        ...
                        # 심각하지 않은 파싱 에러는 디버그로 낮추거나 무시
                        # self.get_logger().warn(f"JSON Logic Error: {e}")
                        pass
